---
title: "Some special Continuous random variables"
editor_options: 
  chunk_output_type: console
bibliography: references.bib
---

Like the discrete case, varieties of real life events can be described by relatively few families of continuous distributions. In this section we will discuss Uniform, Normal, Exponential and Gamma families.
...
![Graph of f(x)](images/clipboard-3046233387.png){#fig-unifpdf width="377" fig-align="left"}
...
![Computing area for an interval of Uniform distribution](images/clipboard-2570030964.png){#fig-unifarea width="380" fig-align="left"}
...
(b) Here, $5 \ \ mins=\frac{5}{60} hr=\frac{1}{12} hr$.

    We know, $F(t)=1-e^{-\lambda t}; t>0$
...
*P(more than 5 seconds will elapse before the next emission)*=

$P(T>5)=e^{-\lambda * 5}=e^{-0.25*5}=0.2865$
...
**a)** Let, $T=time \ \ between \ \ requests \ \ in \ \ second$
...
$\therefore \lambda=2 s^{-1}$.

-------------------------------------------------------------------

**b)** If $M$ is the median time between request then, $P(T \le M)=0.5$

$\implies F(M)=0.5$

$\implies 1-e^{-\lambda *M}=0.5$ $\implies e^{-\lambda *M}=0.5$
...
$\implies 1-e^{-\lambda \times P_{80}}=0.80$
...
$P(T>1+2 /T>2)=P(T>1)=e^{-\lambda \times1}=0.1353$ \[By using Lack of memory property\]
...
f.  If 3 seconds have elapsed with no emission, what is the probability that there will be an emission within the next second? \[Hint: *Use Lack of Memory Property*\]
...
$P(T<1)=F(1)=1-e^{-\lambda \times 1}$ (*do it yourself*)
